...
const SEVERITIES = [
  { id: 'critical', label: 'Crítica', color: 'bg-red-500' },
...
export const FloatingFilters = () => {
  ...
  const { hiddenFilters, toggleFilter, hiddenSeverities, toggleSeverityFilter } = useFilterStore();
  ...
  return (
    <div className="absolute right-4 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-4 max-h-[90vh] overflow-y-auto no-scrollbar pointer-events-auto">
      ...
      <div className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md p-2 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-800 flex flex-col gap-2">
        {SEVERITIES.map((sev) => {
          const isActive = !hiddenSeverities.includes(sev.id);
          return (
            <button
              key={sev.id}
              onClick={() => toggleSeverityFilter(sev.id)}
              title={sev.label}
              className={`w-11 h-11 flex items-center justify-center rounded-xl text-xs font-bold transition-all border ${
                isActive 
                  ? `${sev.color} border-transparent text-white shadow-md scale-105` 
